...
import com.auralis.emulator.utils.RemovableStorageHelper
import com.auralis.emulator.viewmodel.CompressProgressDialogViewModel
import java.lang.ref.WeakReference
import java.util.Date

/**
 * Class which contains methods that interact
 * with the native side of the Auralis code.
 */
object NativeLibrary {
    ...
    val isPortraitMode: Boolean
        get() = AuralisApplication.appContext.resources.configuration.orientation ==
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val alert = MaterialAlertDialogBuilder(requireContext())
                ...
                .setMessage(
                    Html.fromHtml(
                        if (result == ErrorArticDisconnected)
                            AuralisApplication.appContext.resources.getString(R.string.artic_server_comm_error)
                        else
                            AuralisApplication.appContext.resources.getString(R.string.redump_games),
    ...
    /**
     * Logs the Auralis version, Android version and, CPU.
     */
    ...
    fun createFile(directory: String, filename: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            AuralisApplication.documentsTree.createFile(directory, filename)
        } else {
    ...
    fun createDir(directory: String, directoryName: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            try {
                AuralisApplication.documentsTree.createDir(directory, directoryName)
            } catch (e: Exception) {
                false
            }
        } else {
    ...
    fun openContentUri(path: String, openMode: String): Int =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.openContentUri(path, openMode)
        } else {
    ...
    fun getFilesName(path: String): Array<String?> =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.getFilesName(path)
        } else {
    ...
    fun getSize(path: String): Long =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.getFileSize(path)
        } else {
    ...
    fun fileExists(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.exists(path)
        } else {
    ...
    fun isDirectory(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.isDirectory(path)
        } else {
    ...
    fun copyFile(
        ...
        if (FileUtil.isNativePath(sourcePath) &&
            ...
            AuralisApplication.documentsTree
    ...
    fun renameFile(path: String, destinationFilename: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            try {
                AuralisApplication.documentsTree.renameFile(path, destinationFilename)
            } catch (e: Exception) {
                false
            }
        } else {
    ...
    fun updateDocumentLocation(sourcePath: String, destinationPath: String): Boolean =
        AuralisApplication.documentsTree.updateDocumentLocation(sourcePath, destinationPath)
    ...
    fun moveFile(filename: String, sourceDirPath: String, destinationDirPath: String): Boolean =
        if (FileUtil.isNativePath(sourceDirPath)) {
            try {
                AuralisApplication.documentsTree.moveFile(filename, sourceDirPath, destinationDirPath)
            } catch (e: Exception) {
                false
            }
        } else {
    ...
    fun deleteDocument(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            AuralisApplication.documentsTree.deleteDocument(path)
        } else {
