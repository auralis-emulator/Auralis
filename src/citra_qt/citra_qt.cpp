...
GMainWindow::GMainWindow(Core::System& system_)
    ...
    for (int i = 1; i < args.size(); ++i) {
        ...
        if (args[i] == QStringLiteral("--version") || args[i] == QStringLiteral("-v")) {
            const std::string version_string = std::string("Auralis ") + Common::g_build_fullname;
    ...
    LOG_INFO(Frontend, "Auralis Version: {} | {}-{}", Common::g_build_fullname, Common::g_scm_branch,
             Common::g_scm_desc);
#if CITRA_ARCH(x86_64)
    ...
    QIcon auralis_icon = QIcon(QString::fromStdString(":/icons/default/256x256/auralis.png"));
    render_window->setWindowIcon(auralis_icon);
    secondary_window->setWindowIcon(auralis_icon);
    ...
    if (gl_renderer.startsWith(QStringLiteral("D3D12"))) {
        ...
        // We can override the version here to allow Auralis to work.
...
void GMainWindow::InitializeHotkeys() {
    ...
    link_action_shortcut(ui->action_Exit, QStringLiteral("Exit Auralis"));
...
void GMainWindow::ConnectMenuEvents() {
    ...
    connect_menu(ui->action_Report_Compatibility, []() {
        QDesktopServices::openUrl(QUrl(QStringLiteral(
            "https://github.com/auralis-emu/compatibility-list/blob/master/CONTRIBUTING.md")));
    });
    ...
    // Help
    connect_menu(ui->action_Open_Auralis_Folder, &GMainWindow::OnOpenAuralisFolder);
    connect_menu(ui->action_Open_Log_Folder, []() {
        QString path = QString::fromStdString(FileUtil::GetUserPath(FileUtil::UserPath::LogDir));
        QDesktopServices::openUrl(QUrl::fromLocalFile(path));
    });
    connect_menu(ui->action_FAQ, []() {
        QDesktopServices::openUrl(QUrl(QStringLiteral("https://auralis-emu.org/pages/faq/")));
    });
    connect_menu(ui->action_About, &GMainWindow::OnMenuAboutAuralis, QAction::AboutRole);
}
...
static std::optional<QDBusObjectPath> HoldWakeLockLinux(u32 window_id = 0) {
    ...
    QVariantMap options = {};
    //: TRANSLATORS: This string is shown to the user to explain why Auralis needs to prevent the
    //: computer from sleeping
    options.insert(QString::fromLatin1("reason"),
                   QCoreApplication::translate("GMainWindow", "Auralis is running an application"));
    ...
    if (result != Core::System::ResultStatus::Success) {
        ...
        case Core::System::ResultStatus::ErrorGetLoader:
            ...
            QMessageBox::critical(
                ...
                tr("Your app format is not supported.<br/>Please follow the guides to redump your "
                   "<a "
                   "href='https://web.archive.org/web/20240304210021/https://auralis-emulator.org/wiki/"
                   "dumping-game-cartridges/'>game "
                   "cartridges</a> or "
                   "<a "
                   "href='https://web.archive.org/web/20240304210011/https://auralis-emulator.org/wiki/"
        ...
        case Core::System::ResultStatus::ErrorLoader_ErrorEncrypted: {
            QMessageBox::critical(this, tr("App Encrypted"),
                                  tr("Your app is encrypted. <br/>"
                                     "<a "
                                     "href='https://auralis-emu.org/blog/game-loading-changes/'>"
        ...
        case Core::System::ResultStatus::ErrorLoader_ErrorGbaTitle:
            QMessageBox::critical(this, tr("Unsupported App"),
                                  tr("GBA Virtual Console is not supported by Auralis."));
            break;
...
void GMainWindow::BootGame(const QString& filename) {
    ...
    LOG_INFO(Frontend, "Auralis starting...");
    if (!is_artic) {
...
bool GMainWindow::MakeShortcutIcoPath(const u64 program_id, const std::string_view game_file_name,
    ...
    // Get path to Auralis icons directory & icon extension
...
void GMainWindow::OnGameListCreateShortcut(u64 program_id, const std::string& game_path,
    ...
    if (env_flatpak_id) {
        ...
        // Get path to Auralis executable
    ...
    const std::string comment = fmt::format("Start {:s} with the Auralis Emulator", game_title);
...
void GMainWindow::OnMenuSetUpSystemFiles() {
    ...
    QLabel label_description(
        tr("<p>Auralis needs console unique data and firmware files from a real console to be "
           "able to use some of its features.<br>Such files and data can be set up with the <a "
           "href=https://github.com/auralis-emu/ArticSetupTool>Auralis "
           "Artic Setup Tool</a><br>Notes:<ul><li><b>This operation will install console unique "
           "data to Auralis, do not share your user or nand folders<br>after performing the setup "
           "process!</b></li><li>While doing the setup process, Auralis will link to the console "
           "running the setup tool. You can unlink the<br>console later from the System tab in the "
           "emulator configuration menu.</li><li>Do not go online with both Auralis and your 3DS "
    ...
    QLabel label_enter(tr("Enter Auralis Artic Setup Tool address:"), &dialog);
...
void GMainWindow::UninstallTitles(
    ...
    if (failed) {
        QMessageBox::critical(this, tr("Auralis"), tr("Failed to uninstall '%1'.").arg(failed_name));
    } else if (!future_watcher.isCanceled()) {
        QMessageBox::information(this, tr("Auralis"),
...
void GMainWindow::OnOpenAuralisFolder() {
    ...
    if (result == QMessageBox::Help) {
        QDesktopServices::openUrl(
            QUrl(QStringLiteral("https://web.archive.org/web/20240301121456/https://"
                                "auralis-emulator.org/wiki/installing-ffmpeg-for-the-video-dumper/")));
#ifdef _WIN32
...
void GMainWindow::OnOpenFFmpeg() {
    ...
    for (auto& library_name : library_names) {
        if (!FileUtil::Exists(bin_dir + DIR_SEP + library_name)) {
            QMessageBox::critical(this, tr("Auralis"),
    ...
    if (success.load()) {
        QMessageBox::information(this, tr("Auralis"), tr("FFmpeg has been sucessfully installed."));
    } else {
        QMessageBox::critical(this, tr("Auralis"),
    ...
    if (result == Core::System::ResultStatus::ErrorSystemFiles) {
        const QString common_message =
            tr("%1 is missing. Please <a "
               "href='https://web.archive.org/web/20240304201103/https://auralis-emulator.org/wiki/"
        ...
        message = tr("A fatal error occurred. "
                     ...
                     "community.auralis-emulator.org/t/how-to-upload-the-log-file/296'>Check "
...
void GMainWindow::OnMenuAboutAuralis() {
...
bool GMainWindow::ConfirmClose() {
    ...
    QMessageBox::StandardButton answer =
        QMessageBox::question(this, tr("Auralis"), tr("Would you like to exit now?"),
                              QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
...
void GMainWindow::OnEmulatorUpdateAvailable() {
    ...
    update_prompt.setText(tr("Update %1 for Auralis is available.\nWould you like to download it?")
                              .arg(version_string));
    update_prompt.exec();
    if (update_prompt.button(QMessageBox::Yes) == update_prompt.clickedButton()) {
        std::string update_page_url;
        if (ShouldCheckForPrereleaseUpdates()) {
            update_page_url = "https://github.com/auralis-emu/auralis/releases";
        } else {
            update_page_url = "https://auralis-emu.org/pages/download/";
        }
...
void GMainWindow::UpdateWindowTitle() {
    ...
    if (game_title.isEmpty()) {
        setWindowTitle(QStringLiteral("Auralis %1").arg(full_name));
    } else {
        setWindowTitle(QStringLiteral("Auralis %1 | %2").arg(full_name, game_title));
        render_window->setWindowTitle(
            QStringLiteral("Auralis %1 | %2 | %3").arg(full_name, game_title, tr("Primary Window")));
        secondary_window->setWindowTitle(QStringLiteral("Auralis %1 | %2 | %3")
...
int LaunchQtFrontend(int argc, char* argv[]) {
    ...
    QCoreApplication::setOrganizationName(QStringLiteral("Auralis Emulator Developers"));
    QCoreApplication::setOrganizationDomain(QStringLiteral("auralis-emulator.com"));
    QCoreApplication::setApplicationName(QStringLiteral("Auralis"));
    QGuiApplication::setDesktopFileName(QStringLiteral("org.auralis_emu.Auralis"));
