; Copyright Dolphin Emulator Project / Auralis Emulator Project
...
!define PRODUCT_NAME "Auralis"
!define PRODUCT_PUBLISHER "Auralis Emulator Developers"
!define PRODUCT_WEB_SITE "https://auralis-emulator.com/"
...
Name "${PRODUCT_NAME}"
OutFile "auralis-${PRODUCT_VERSION}-windows-${PRODUCT_VARIANT}-installer.exe"
...
; MUI Settings
!define MUI_ICON "../../dist/auralis.ico"
...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\auralis.exe"
...
Section "Base"
  ...
  CreateShortCut "$SMPROGRAMS\$DisplayName.lnk" "$INSTDIR\auralis.exe"
  ${If} $DesktopShortcut == 1
    CreateShortCut "$DESKTOP\$DisplayName.lnk" "$INSTDIR\auralis.exe"
  ${EndIf}
SectionEnd
...
Section -Post
  ...
  WriteRegStr SHCTX "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\auralis.exe"
  ...
  WriteRegStr SHCTX "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\auralis.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\*.dll"
  Delete "$INSTDIR\auralis.exe"
  Delete "$INSTDIR\auralis-room.exe"
