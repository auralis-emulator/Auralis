...
void CubebInput::StartSampling(const InputParameters& params) {
    ...
    auto init_result = cubeb_init(&impl->ctx, "Auralis Input", nullptr);
    ...
    auto stream_init_result = cubeb_stream_init(
        impl->ctx, &impl->stream, "Auralis Microphone", input_device, &input_params, nullptr,
...
std::vector<std::string> ListCubebInputDevices() {
    ...
    if (cubeb_init(&ctx, "Auralis Input Device Enumerator", nullptr) != CUBEB_OK) {
