...
import com.auralis.emulator.utils.PermissionsHandler.hasWriteAccess
import java.io.File
 ...
 * from the Auralis APK to the external file system.
 */
object DirectoryInitialization {
    ...
    val internalUserPath: String
        get() = AuralisApplication.appContext.filesDir.canonicalPath
    private val isAuralisDirectoryInitializationRunning = AtomicBoolean(false)

    val context: Context get() = AuralisApplication.appContext

    @JvmStatic
    fun start(): DirectoryInitializationState? {
        if (!isAuralisDirectoryInitializationRunning.compareAndSet(false, true)) {
            return null
        }

        if (directoryState != DirectoryInitializationState.CITRA_DIRECTORIES_INITIALIZED) {
            directoryState = if (hasWriteAccess(context)) {
                if (setAuralisUserDirectory()) {
                    AuralisApplication.documentsTree.setRoot(Uri.parse(userPath))
        ...
        isAuralisDirectoryInitializationRunning.set(false)
    ...
    @JvmStatic
    fun areAuralisDirectoriesReady(): Boolean {
        return directoryState == DirectoryInitializationState.CITRA_DIRECTORIES_INITIALIZED
    }

    fun resetAuralisDirectoryState() {
        directoryState = null
        isAuralisDirectoryInitializationRunning.compareAndSet(true, false)
    }
    ...
    val userDirectory: String?
        get() {
            ...
            check(!isAuralisDirectoryInitializationRunning.get()) {
                "DirectoryInitialization has to finish running first!"
            }
            return userPath
        }

    fun setAuralisUserDirectory(): Boolean {
        val dataPath = PermissionsHandler.citraDirectory
        if (dataPath.toString().isNotEmpty()) {
            userPath = dataPath.toString()
            android.util.Log.d("[Auralis Frontend]", "[DirectoryInitialization] User Dir: $userPath")
            return true
        }
        return false
    }
