...
package com.auralis.emulator.adapters
...
import com.auralis.emulator.HomeNavigationDirections
import com.auralis.emulator.AuralisApplication
import com.auralis.emulator.NativeLibrary
import com.auralis.emulator.R
import com.auralis.emulator.adapters.GameAdapter.GameViewHolder
import com.auralis.emulator.databinding.CardGameBinding
import com.auralis.emulator.databinding.DialogShortcutBinding
import com.auralis.emulator.features.cheats.ui.CheatsFragmentDirections
import com.auralis.emulator.fragments.IndeterminateProgressDialogFragment
import com.auralis.emulator.model.Game
import com.auralis.emulator.utils.FileUtil
import com.auralis.emulator.utils.GameIconUtils
import com.auralis.emulator.viewmodel.GamesViewModel
...
class GameAdapter(
    ...
    private val preferences: SharedPreferences
        get() = PreferenceManager.getDefaultSharedPreferences(AuralisApplication.appContext)
    ...
    private fun showOpenContextMenu(view: View, game: Game) {
        ...
        val popup = PopupMenu(view.context, view).apply {
            ...
            listOf(
                ...
                menu.findItem(id)?.isEnabled =
                    AuralisApplication.documentsTree.folderUriHelper(dir)?.let {
        ...
        popup.setOnMenuItemClickListener { menuItem ->
            ...
            val uri = when (menuItem.itemId) {
                R.id.game_context_open_app -> AuralisApplication.documentsTree.folderUriHelper(dirs.appDir)
                R.id.game_context_open_save_dir -> AuralisApplication.documentsTree.folderUriHelper(dirs.saveDir)
                R.id.game_context_open_updates -> AuralisApplication.documentsTree.folderUriHelper(dirs.updatesDir)
                R.id.game_context_open_dlc -> AuralisApplication.documentsTree.folderUriHelper(dirs.dlcDir)
                R.id.game_context_open_extra -> AuralisApplication.documentsTree.folderUriHelper(dirs.extraDir)
                R.id.game_context_open_textures -> AuralisApplication.documentsTree.folderUriHelper(dirs.texturesDir, true)
                R.id.game_context_open_mods -> AuralisApplication.documentsTree.folderUriHelper(dirs.modsDir, true)
                else -> null
            }
        ...
        popup.setOnMenuItemClickListener { menuItem ->
            val uninstallAction: () -> Unit = {
                when (menuItem.itemId) {
                    R.id.game_context_uninstall -> AuralisApplication.documentsTree.deleteDocument(dirs.gameDir)
                    R.id.game_context_uninstall_dlc -> FileUtil.deleteDocument(AuralisApplication.documentsTree.folderUriHelper(dirs.dlcDir)
                        .toString())
                    R.id.game_context_uninstall_updates -> FileUtil.deleteDocument(AuralisApplication.documentsTree.folderUriHelper(dirs.updatesDir)
                        .toString())
                }
                ViewModelProvider(activity)[GamesViewModel::class.java].reloadGames(true)
    ...
    private fun refreshShortcutDialogIcon() {
        if (imagePath != null) {
            val originalBitmap = BitmapFactory.decodeStream(
                AuralisApplication.appContext.contentResolver.openInputStream(
                    imagePath!!.toUri()
                )
            )
            val scaledBitmap = {
