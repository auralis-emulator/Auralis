...
CubebSink::CubebSink(std::string_view target_device_name) : impl(std::make_unique<Impl>()) {
    if (cubeb_init(&impl->ctx, "Auralis Output", nullptr) != CUBEB_OK) {
    ...
    auto stream_err = cubeb_stream_init(impl->ctx, &impl->stream, "AuralisAudio", nullptr, nullptr,
...
std::vector<std::string> ListCubebSinkDevices() {
    ...
    if (cubeb_init(&ctx, "Auralis Output Device Enumerator", nullptr) != CUBEB_OK) {
        LOG_CRITICAL(Audio_Sink, "cubeb_init failed");
        return {};
    }
