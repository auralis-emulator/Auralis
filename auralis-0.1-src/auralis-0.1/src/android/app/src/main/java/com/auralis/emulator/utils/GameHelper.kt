...
import com.auralis.emulator.model.GameInfo
...
object GameHelper {
    ...
    fun getGames(): List<Game> {
        ...
        val context = AuralisApplication.appContext
    ...
    fun getGame(uri: Uri, isInstalled: Boolean, addedToLibrary: Boolean): Game {
        ...
        val newGame = Game(
            ...
            gameInfo?.getCompany() ?: "",
            if (isEncrypted) { AuralisApplication.appContext.getString(R.string.unsupported_encrypted) } else { gameInfo?.getRegions() ?: "" },
            isInstalled,
            ...
            if (FileUtil.isNativePath(filePath)) {
                AuralisApplication.documentsTree.getFilename(filePath)
            } else {
